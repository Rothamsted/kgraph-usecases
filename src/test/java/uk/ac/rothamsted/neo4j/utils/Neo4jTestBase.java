...
public abstract class Neo4jTestBase
{
	 ...
	 * 
	 * All static stuff are run once only when the JVM loads the class.
	 * 
	 */
	protected static Driver driver = GraphDatabase.driver( 
		Neo4jConnection.TEST_URL, AuthTokens.basic(Neo4jConnection.TEST_USER, Neo4jConnection.TEST_PWD) 
	);

	/**
	 * We call the init methods below in one {code @BeforeClass} method, since it's hard to control the
	 * order of this annotation, when it's used multiple times (it's alphabetical).
	 */
	@BeforeClass
	...
	private static void createData()
	{
		...
		out.println ( "Test data created" );
	} 
	
	
	/**
	 * TODO: This is run only once when the JVM quits. Tricks might be needed to run this only once.
	 *  
	 */
	@AfterClass
